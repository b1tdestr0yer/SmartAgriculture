...
########################################################################################################################################################################################################################
    ...
    {
        $selectQuery = "SELECT username, password, isDisabled FROM userData WHERE (username='" . $_POST['username'] . "' AND password='" . $_POST['password'] . "');";
        ...
        if($result->num_rows > 0)
        {
            while($row = $result->fetch_assoc())
            {
                ...
                if(!isset($_COOKIE[$cookie_name]))
                {
                    ...
                    setcookie($cookie_name, $cookie_value, time() + (30 * 60), '/');    // Cookie duration: 30 mins.
                }
            }
        }
        ...
        {
            $errorMsg = "There is no user with this username/password.";
        }
    }
...
<html>
    ...
    <?php
        ...
        if(isset($_COOKIE[$cookie_name]))
        {
            echo "Cookie se cheama: " . $cookie_name . " si are valoarea: " . $cookie_value . ".<br><br>";
        }
        else
        {
            echo "Iaca pozna!";
...
<body>
    <?php 
        ...
        {
            if(isset($_COOKIES["userID"]))
            {
                echo "<script>alert('SUPEB');</script>";
            }

            else
            {
                echo "<script>alert('MORB');</script>";
